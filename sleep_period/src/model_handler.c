...
// 	dk_set_led(led_idx, true); /* true = SÁNG */
...
// /* ====================================================================
//  * CÁC BIẾN CẤU HÌNH QUẢN LÝ CHU KỲ NĂNG LƯỢNG THỨC / NGỦ
//  * ==================================================================== */
...
// static bool cycle_mode_active = false;      /* TRẠNG THÁI BAN ĐẦU: CHƯA KÍCH HOẠT CHU KỲ NGỦ (LOGIC BÌNH THƯỜNG) */
// static bool is_awake = true;                /* Trạng thái Thức/Ngủ trong chu kỳ */
// static bool led2_blink_state = false;       /* Trạng thái toggle của LED2 */

// /* Hàm xử lý nhấp nháy nhanh LED2 (chỉ chạy khi ở chế độ THỨC) */
...
// 	dk_set_led(2, led2_blink_state); /* Điều khiển trực tiếp chân LED3 (index 2) */

// 	/* Nhấp nháy nhanh với chu kỳ 150ms */
// 	k_work_reschedule(&blink_work, K_MSEC(150));
// }
...
// 	if (is_awake) {
// 		/* ----------------------------------------------------
// 		 * CHẾ ĐỘ THỨC 10S: Bluetooth Mesh hoạt động bình thường
// 		 * ---------------------------------------------------- */
// 		/* 1. Khôi phục lại trạng thái bật/tắt các đèn LED theo đúng lệnh cuối cùng từ nRF Mesh */
// 		for (int i = 0; i < ARRAY_SIZE(led_ctx); i++) {
// 			if (i != 2) { /* Ngoại trừ LED2 đang được dùng để nhấp nháy */
// 				dk_set_led(i, led_ctx[i].value);
// 			}
// 		}

// 		/* 2. Kích hoạt lại tiến trình nhấp nháy nhanh cho LED2 */
// 		k_work_reschedule(&blink_work, K_NO_WAIT);

// 	} else {
// 		/* ----------------------------------------------------
// 		 * CHẾ ĐỘ NGỦ 10S: Tắt toàn bộ phần cứng để hạ dòng tĩnh về 2-4uA
// 		 * ---------------------------------------------------- */
// 		/* 1. Dừng việc hẹn giờ nhấp nháy của LED2 */
// 		k_work_cancel_delayable(&blink_work);

// 		/* 2. ÉP TẮT TOÀN BỘ ĐÈN LED TRÊN KIT (false = TẮT đốí với thư viện dk) */
// 		dk_set_led(0, false); 
// 		dk_set_led(1, false);
// 		dk_set_led(2, false);
// 		dk_set_led(3, false);
...
// /* Hàm nhận lệnh điều khiển OnOff từ ứng dụng nRF Mesh */
// static void led_set(struct bt_mesh_onoff_srv *srv, struct bt_mesh_msg_ctx *ctx,
// 		    const struct bt_mesh_onoff_set *set,
// 		    struct bt_mesh_onoff_status *rsp)
// {
...
// 	/* [CỰC KỲ QUAN TRỌNG]: NẾU NHẬN ĐƯỢC LỆNH ĐẦU TIÊN TỪ APP, CHUYỂN SANG CHẾ ĐỘ CHU KỲ */
// 	if (!cycle_mode_active) {
// 		cycle_mode_active = true;
// 		is_awake = true;
// 		k_work_reschedule(&cycle_work, K_MSEC(10000)); /* Bắt đầu đếm 10s thức đầu tiên */
// 		k_work_reschedule(&blink_work, K_NO_WAIT);     /* Bật LED2 nhấp nháy luôn */
// 	}
...
// 	/* Luôn lưu trạng thái mong muốn từ App vào bộ nhớ đệm */
// 	led->value = set->on_off; 

// 	if (!bt_mesh_model_transition_time(set->transition)) {
// 		led->remaining = 0;
		
// 		/* CHỈ xuất trực tiếp ra chân vật lý nếu đang trong chu kỳ THỨC và không phải LED2 */
...
// 	} else {
// 		dk_set_leds(0x00); /* Tắt sạch đèn khi hết chế độ Attention */
// 	}
// }
...
// 	/* Khởi tạo cấu trúc luồng công việc định thời */
// 	k_work_init_delayable(&blink_work, blink_handler);
// 	k_work_init_delayable(&cycle_work, cycle_handler);

// 	/* TRẠNG THÁI BAN ĐẦU: Khóa chế độ chu kỳ, tắt toàn bộ LED để sẵn sàng chờ nạp Mesh */
...
#define LED_BO_MACH_SANG   false
#define LED_BO_MACH_TAT    true

static void led_transition_start(struct led_ctx *led)
{
	int led_idx = led - &led_ctx[0];
	/* Khi có lệnh transition bật nguồn, gán trạng thái SÁNG chuẩn xác cho bo mạch */
	dk_set_led(led_idx, LED_BO_MACH_SANG); 
	k_work_reschedule(&led->work, K_MSEC(led->remaining));
	led->remaining = 0;
}
...
static void blink_handler(struct k_work *work)
{
	...
	dk_set_led(2, led2_blink_state ? LED_BO_MACH_SANG : LED_BO_MACH_TAT);
...
static void cycle_handler(struct k_work *work)
{
	...
	if (is_awake) {
		...
		for (int i = 0; i < ARRAY_SIZE(led_ctx); i++) {
			if (i != 2) { 
				dk_set_led(i, led_ctx[i].value ? LED_BO_MACH_SANG : LED_BO_MACH_TAT);
			}
		}
		    ...
		    struct bt_mesh_onoff_status *rsp)
{
	 ...
	 * ---------------------------------------------------------------- */
		...
		if (!bt_mesh_model_transition_time(set->transition)) {
			...
			if (is_awake) {
				dk_set_led(led_idx, set->on_off ? LED_BO_MACH_SANG : LED_BO_MACH_TAT);
			}
		} else {
			led->remaining = set->transition->time;
			if (set->transition->delay) {
...
static void led_work(struct k_work *work)
{
	...
	if (led->remaining) {
		...
		if (is_awake && led_idx != 2) {
			dk_set_led(led_idx, led->value ? LED_BO_MACH_SANG : LED_BO_MACH_TAT);
		}
...
const struct bt_mesh_comp *model_handler_init(void)
{
	...
	for (int i = 0; i < ARRAY_SIZE(led_ctx); ++i) {
		k_work_init_delayable(&led_ctx[i].work, led_work);
		led_ctx[i].value = 0; /* Mặc định ban đầu lưu trong bộ nhớ đệm là TẮT (0) */
	}
	...
	/* ----------------------------------------------------
	 * MẶC ĐỊNH BAN ĐẦU: Chưa bật chu kỳ, tất cả LED đều TẮT giải phóng dòng
	 * ---------------------------------------------------- */
	cycle_mode_active = false;
	is_awake = true;
	
	for (int i = 0; i < ARRAY_SIZE(led_ctx); i++) {
		dk_set_led(i, LED_BO_MACH_TAT); /* Đảm bảo tắt hoàn toàn 4 LED */
	}
